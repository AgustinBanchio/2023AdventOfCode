...
func main() {
	...
	fmt.Println("Advent of Code 2023 - Day 16 - Part Two")
	...
	for x := 0; x < len(matrix); x++ {
		for y := 0; y < len(matrix[0]); y++ {
			...
			if x == 0 {
				resetTiles(matrix)
				processBeam(matrix, x, y, "right")
				current := countEnergized(matrix)
				if current > total {
					total = current
				}
			}
...
func resetTiles(matrix [][]*tile) {
	for y := 0; y < len(matrix[0]); y++ {
		for x := 0; x < len(matrix); x++ {
			// reset tile
			matrix[x][y].energized = false
			matrix[x][y].directions = []string{}
		}
	}
}

func countEnergized(matrix [][]*tile) int {
