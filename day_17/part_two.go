...
func main() {
	...
	fmt.Println("Advent of Code 2023 - Day 17 - Part Two")
...
func calculateHeatLoss(matrix [][]int) int {
	...
	for currentTile, noMore := pq.Pop(); !noMore; currentTile, noMore = pq.Pop() {
		...
		if currentTile.x == len(matrix)-1 && currentTile.y == len(matrix[0])-1 && currentTile.timesInDir >= 4 {
		...
		if currentTile.timesInDir < 10 && currentTile.dir != "" {
		...
		if currentTile.timesInDir >= 4 || currentTile.dir == "" {
